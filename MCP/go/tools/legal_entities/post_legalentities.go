...
func Post_legalentitiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.LegalEntityInfoRequiredType
		...
		url := fmt.Sprintf("%s/legalEntities", cfg.BaseURL)
...
func CreatePost_legalentitiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_legalEntities",
		mcp.WithDescription("Create a legal entity"),
		mcp.WithString("reference", mcp.Description("Input parameter: Your reference for the legal entity, maximum 150 characters.")),
		mcp.WithString("type", mcp.Required(), mcp.Description("Input parameter: The type of legal entity.\n\n Possible values: **individual** or **organization**")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_legalentitiesHandler(cfg),
	}
}
